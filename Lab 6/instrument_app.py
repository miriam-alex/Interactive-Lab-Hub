...
# Use threading async mode to avoid eventlet monkey-patch ordering issues
...
def instrument():
    """Serve the instrument events coordinator page"""
...
if __name__ == '__main__':
    print("=" * 60)
    print("  Instrument Coordinator Server")
    print("=" * 60)
    print(f"  Instrument UI:      http://0.0.0.0:5000/instrument")
    print(f"  Controller:         http://0.0.0.0:5000/controller")
    print("=" * 60)
    
    # Start the instrument MQTT bridge (kitchen-instrument)
    try:
        from mqtt_bridge_instrument import start_mqtt_bridge
        # Provide an empty dict for instrument state (bridge will populate userdata)
        start_mqtt_bridge(socketio, {})
    except ImportError:
        print("  MQTT instrument bridge not available (install paho-mqtt)")
    except Exception as e:
        print(f"  MQTT instrument bridge disabled: {e}")
